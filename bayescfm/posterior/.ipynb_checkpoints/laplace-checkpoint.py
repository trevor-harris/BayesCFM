...
class LaplaceCFM:
    ...
    def posterior_sample(
        ...
        *,
        ...
        """
        Return a list of `n` independent model draws from the Laplace posterior.
        Each draw is a deep-copied nn.Module with a sampled state_dict loaded.
        Requires `fit()` to have been called.
        """
        assert getattr(self, "_fitted", False), "call fit() before sampling models"
        if seed is not None:
            torch.manual_seed(int(seed))
        base_device = device or next(self.model.parameters()).device
        models = []
        for k in range(n):
            sd = self.sample_state_dict()              # sampled weights (MAP + delta)
            m  = copy.deepcopy(self.model).to(base_device)
    ...
    # ---------pointwise posterior prediction ----------------

    @torch.no_grad()
    def posterior_sample_pointwise(self, x: torch.Tensor, t: torch.Tensor, y: torch.Tensor | None = None,
